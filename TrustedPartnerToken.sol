...
contract cartractcoin is erc20_interface{
    ...
    //erlaubt es von einem fremden Konto zu überweisen; ACHTUNG MUSS EXTERNAL bleiben und msg sender validieren
    function approveFromAddress(address payable tokenOwner,address payable spender, uint tokens) external returns (bool success){
        //address tokenOwner= msg.sender;
        require(balanceOf(tokenOwner) >= tokens);
        allowed[tokenOwner][spender] = tokens;
        emit Approval(tokenOwner, spender, tokens ); // Event
